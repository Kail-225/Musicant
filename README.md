Данный бот создан в качестве радио с песнями. Громкость увы у каждого из них сильно разнится(учитывая, что есть фонк это неудивительно) + я настраивал под свои уши, которые сидят не на 100% громкости.<br>
**Поэтому если сидите на 100% громкости, то ставьте громкость бота на ```60%```**<br>
А если на 45%, то можете оставить ```100%```<br>
**```Список загруженных треков(формат: название - автор):```**<br>
```Royalty - EgzodMaestro и ChivesNeoni Instrumental version```<br>
```Прощание - Три дня дождя```<br>
```Faster_n_harder - 6arelyhuman```<br>
```Rave - Dxrk```<br>
```Dare - Sayfalse и TRXVELER и DJALIM```<br>
```Children_of_the_sky - Imagine Dragons```<br>
```Suzume - RADWIMPS feat toaka```<br>
```Shinunoga_E-wa - Fujii_Kaze```<br>
```Kiss_of_death - Sati Akura```<br>
```Katyusha - Sati Akura```<br>
```Suzume(кавер на русском)- Sati Akura```<br>
```Hope_Is_the_Thing_with_Feathers - Sati Akura```<br>
```Idol- Sati Akura```<br>
```Starfall - Sati Akura```<br>
```White_Night - Sati Akura```<br>
```Wildfire - Sati Akura```<br>
```Shinunoga_E-wa(кавер на русском)- Sati Akura```<br>
```La_vaguelette(кавер на английском) - Sati Akura```<br>
```Earth - Jesper Kyd```<br>
```La_vaguelette - Ccilia Cara feat Genshin Impact```<br>
```Honor_for_all - Jon Licht и Daniel Licht```<br>
```Take_the_Journey - Hoyo-Mix и Anthony Lynch (или Honkai Star Rail 1.0 OST. Я сам не понимаю. Больше смахивает на название альбома)```<br>
```Rogue_Main_Theme - Elitsa Alexandrova```<br>
```Main_Title - Ramin Djawadi (хороший сериал. Жаль запороли конец. Созвать знамёна!)```<br>
```Janissary_Song_1 - Soner Akaln```<br>
```Janissary_Song_2 - Soner Akaln```<br>
```Deepest_Love - Aytekin Ata-Ak```
**Если увидите ошибки или найдёте дополнения, то буду рад увидеть обновления**
